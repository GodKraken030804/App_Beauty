...
class CursosAdministradoresView extends StatefulWidget {
  const CursosAdministradoresView({super.key});

  @override
  State<CursosAdministradoresView> createState() =>
      _CursosAdministradoresViewState();
}

class _CursosAdministradoresViewState extends State<CursosAdministradoresView>
  ...
  // Widget animado para botones y tarjetas
  ...
  // Función para mostrar notificaciones con estilo de gradiente
  ...
  // Función para obtener los cursos asignados al usuario actual
  Future<void> _fetchCursosAsignados() async {
    ...
    try {
      ...
      if (asignacionesResponse.statusCode == 200) {
        ...
        // Filtrar las asignaciones del usuario actual
        ...
        if (cursosResponse.statusCode == 200) {
          ...
          // Filtrar los cursos que están asignados al usuario y MERGEAR el 'excel' desde la asignación
          final cursosDelUsuario = todosLosCursos.where((curso) {
            return asignacionesUsuario
                .any((asignacion) => asignacion['id_curso'] == curso['id']);
          }).map((curso) {
            final asign = asignacionesUsuario.firstWhere(
              (a) => a['id_curso'] == curso['id'],
              orElse: () => {},
            );
            final Map<String, dynamic> merged =
                Map<String, dynamic>.from(curso);
            if (asign is Map && asign['excel'] != null) {
              merged['excel'] = asign['excel'];
            }
            return merged;
          }).toList();
  ...
  // Widget para el botón de acción flotante
              ...
              child: Column(
                children: [
                  ...
                  if (isLoading)
                    ...
                    // Widget animado para estado vacío
                    _buildAnimatedButton(
                      onTap: () {},
                      child: Padding(
                        padding: EdgeInsets.all(24),
